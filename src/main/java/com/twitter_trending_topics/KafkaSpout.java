...
public abstract class KafkaSpout extends BaseRichSpout implements Runnable {
  ...
  public void activate() {
    ...
    if (streams == null) {
      ...
      topicCountMap.put("raw_data", new Integer(1));
      Map<String, List<KafkaStream<byte[], byte[]>>> consumerMap =
          consumerConnector.createMessageStreams(topicCountMap);
      streams = consumerMap.get("raw_data");
    }
  }
  ...
  public void open(Map conf, TopologyContext context, SpoutOutputCollector collector) {
    ...
    logger.info(" topic = " + "raw_data");
    this.spoutName = String.format("%s-%d", context.getThisComponentId(), context.getThisTaskId());
    logger.info("Opened3 : {}", this.spoutName);
    Properties kafkaProperties = new Properties();
    kafkaProperties.put("zookeeper.connect", "localhost:2181");
    kafkaProperties.put("group.id", "testConsumer");
    kafkaProperties.put("zookeeper.session.timeout.ms", "10000");
    kafkaProperties.put("zookeeper.sync.time.ms", "10000");
    kafkaProperties.put("auto.commit.interval.ms", "10000");
    kafkaProperties.put("consumer.timeout.ms", "10000");
  ...
  public void nextTuple() {
    logger.info("nextTuple called");
    checkReaderRunning();
    final byte[] message = getMessage();
    if (message != null) {
      logger.info("streams iterator has next");
      processMessage(message, collector);
    }
  }

  private void checkReaderRunning() {
    logger.info(" Method : checkReaderRunning");
